...
def load_prep_img(data_dir, imname, spacing, slice_intv, do_clip=False, num_slice=3,
                  is_train=False):
    """load volume, windowing, interpolate multiple slices, clip black border, resize according to spacing"""
    im, mask = load_multislice_img_16bit_png(data_dir, imname, slice_intv,
                                             do_clip, num_slice)

    im = windowing(im, WINDOWING)

    if do_clip:  # clip black border
        c = get_range(mask, margin=0)
        if DATA_AUG_POSITION is not False and is_train:
    ...
    if spacing is not None and NORM_SPACING > 0:  # spacing adjust, will overwrite simple scaling
        im_scale = float(spacing) / NORM_SPACING
    else:
        im_scale = float(MAX_IM_SIZE) / float(np.max(im_shape))  # simple scaling

    if DATA_AUG_SCALE is not False and is_train:
        aug_scale = DATA_AUG_SCALE
    ...
    if im_scale != 1:
        im = cv2.resize(im, None, fx=np.float(im_scale),
                        fy=np.float(im_scale),
